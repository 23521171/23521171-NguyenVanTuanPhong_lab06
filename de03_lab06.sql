...
SELECT TOP 1 DUOCPHAM.MADP, TENDP, SUM(SOLUONG) AS SOLUONGNHAP
FROM DUOCPHAM, PHIEUNHAP, CTPN
WHERE DUOCPHAM.MADP = CTPN.MADP AND PHIEUNHAP.SOPN = CTPN.SOPN AND YEAR(NGNHAP) =2017
GROUP BY DUOCPHAM.MADP, TENDP
ORDER BY SOLUONGNHAP DESC

SELECT DUOCPHAM.MADP, TENDP
FROM NHACUNGCAP, DUOCPHAM, PHIEUNHAP, CTPN
WHERE NHACUNGCAP.MANCC = PHIEUNHAP.MANCC AND DUOCPHAM.MADP = CTPN.MADP AND PHIEUNHAP.SOPN = CTPN.SOPN 
AND LOAICC = 'Thuong xuyen' AND DUOCPHAM.MADP NOT IN (
...
WHERE NCC.LOAICC = N'Thuong xuyen' 
  AND DP.MADP NOT IN (
      SELECT DISTINCT DP2.MADP
      FROM DUOCPHAM DP2
      JOIN CTPN CT2 ON DP2.MADP = CT2.MADP
      JOIN PHIEUNHAP PN2 ON CT2.SOPN = PN2.SOPN
      JOIN NHACUNGCAP NCC2 ON PN2.MANCC = NCC2.MANCC
      WHERE NCC2.LOAICC = N'Vang lai'
  );
...
WHERE NOT EXISTS (
    ...
    AND NOT EXISTS (
        SELECT *
        FROM PHIEUNHAP, CTPN
        WHERE YEAR (NGNHAP) = 2017 AND NHACUNGCAP.MANCC = PHIEUNHAP.MANCC AND DUOCPHAM.MADP = CTPN.MADP
        AND CTPN.SOPN = PHIEUNHAP.SOPN
    )
);
